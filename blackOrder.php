...
if (isset($_POST['orderSubmit'])) {
    ...
    $sql1 = "SELECT * FROM bw_format WHERE  size='$size' AND side='$side' ";
    ...
    function GetImageExtension($imagetype)
    {
        ...
        switch($imagetype)
        {
            case 'blackOrders/bmp': return '.bmp';
            case 'blackOrders/gif': return '.gif';
            case 'blackOrders/jpeg': return '.jpg';
            case 'blackOrders/png': return '.png';
    ...
    if (!empty($_FILES["uploadedImage"]["name"])) {
        ...
        $target_path = "blackOrders/".$imagename;

        if(move_uploaded_file($temp_name, $target_path)) {
            $query_upload="INSERT into bw_order (uEmail,uName,size,side,imagePath,submissionDate,qty,price) VALUES
            ('$email','$name','$size','$side','".$target_path."','".date("Y-m-d")."','$quantity','$total')";
